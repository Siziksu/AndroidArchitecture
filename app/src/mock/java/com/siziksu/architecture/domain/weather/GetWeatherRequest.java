...
public final class GetWeatherRequest {
    ...
    public void subscribe(final Success<OpenWeather> success, final Fail fail, final Done done) {
        ...
        String response = FileUtils.getInstance().getStringFromFile("get_weather/response.json");
        OpenWeather openWeather = Injector.provideGson().fromJson(response, OpenWeather.class);
